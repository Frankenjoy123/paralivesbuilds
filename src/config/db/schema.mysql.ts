...
export const gameCategory = table(
  ...
  (table) => [
    index('idx_game_category_slug').on(table.slug),
    index('idx_game_category_status').on(table.status),
  ]
);
...
export const game = table(
  ...
  {
    id: varchar191('id').primaryKey(),
    categoryId: varchar191('category_id')
      .notNull()
  ...
  (table) => [
    ...
    index('idx_game_category_id').on(table.categoryId),
  ]
);

// ParalivesBuilds Tables
export const creator = table(
  'creator',
  {
    id: varchar191('id').primaryKey(),
    slug: varchar191('slug').unique().notNull(),
    ...
    twitterX: varchar('twitter_x', { length: 255 }),
    buildsCount: int('builds_count').default(0).notNull(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
    deletedAt: timestamp('deleted_at'),
  },
  (table) => [
    index('idx_creator_slug').on(table.slug),
    index('idx_creator_deleted_at').on(table.deletedAt),
  ]
);
...
export const style = table(
  'style',
  {
    ...
    slug: varchar191('slug').unique().notNull(),
    image: text('image'),
    buildCount: int('build_count').default(0).notNull(),
    sortOrder: int('sort_order').default(0).notNull(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
  },
  (table) => [
    index('idx_style_slug').on(table.slug),
    index('idx_style_sort_order').on(table.sortOrder),
  ]
);
...
export const build = table(
  'build',
  {
    ...
    title: varchar('title', { length: 255 }).notNull(),
    description: text('description'),
    styleId: varchar191('style_id').references(() => style.id, {
      onDelete: 'set null',
    }),
    ...
    budget: int('budget'),
    images: text('images'), // JSON string array of R2 keys
    workshopUrl: text('workshop_url'),
    videoUrl: text('video_url'),
    creatorId: varchar191('creator_id')
      .notNull()
  ...
  (table) => [
    index('idx_build_slug').on(table.slug),
    index('idx_build_style_id').on(table.styleId),
    index('idx_build_creator_id').on(table.creatorId),
    index('idx_build_featured').on(table.featured),
    index('idx_build_status').on(table.status),
    index('idx_build_deleted_at').on(table.deletedAt),
  ]
);

export const buildTag = table(
  'build_tag',
  {
    buildId: varchar191('build_id')
      .notNull()
      .references(() => build.id, { onDelete: 'cascade' }),
    tagId: varchar191('tag_id')
      .notNull()
      .references(() => tag.id, { onDelete: 'cascade' }),
  },
  (table) => [
    index('idx_build_tag_build_id').on(table.buildId),
    index('idx_build_tag_tag_id').on(table.tagId),
  ]
);
