...
export async function POST(req: Request) {
  try {
    ...
    await db()
      .update(build)
      .set({ status: BuildStatus.ARCHIVED, updatedAt: new Date() })
      .where(eq(build.id, buildId));

    return respData({ message: 'Build rejected successfully' });
  } catch (e) {
    console.error('reject build failed:', e);
    return respErr('Failed to reject build');
  }
}
