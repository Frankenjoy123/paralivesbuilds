...
import { GameCategoryStatus } from '@/shared/models/game-category-types';
...
export function CategoryTableActions({ category, translations }: TableActionsProps) {
  ...
  const handleAction = (
    ...
    startTransition(async () => {
      try {
        const res = await actionFn(category.id);
        if (res.success) {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[160px]">
        ...
        <DropdownMenuItem asChild>
          <Link
            href={`/admin/game-categories/${category.id}/edit`}
        ...
        {category.status === GameCategoryStatus.PUBLISHED ? (
          <DropdownMenuItem
            className="flex w-full items-center gap-2 cursor-pointer"
            onClick={(e) => {
              e.preventDefault();
              handleAction(unpublishCategoryAction);
            }}
          >
          ...
          <DropdownMenuItem
            ...
            onClick={(e) => {
              e.preventDefault();
              handleAction(publishCategoryAction);
            }}
          >
        ...
        <DropdownMenuItem
          ...
          onClick={(e) => {
            e.preventDefault();
            handleAction(deleteCategoryAction, translations.confirmDelete);
          }}
        >
